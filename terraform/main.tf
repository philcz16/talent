...
resource "azurerm_virtual_machine_extension" "test" {
  ...
  location             = "${azurerm_resource_group.vm_rg.location}"
  resource_group_name  = "${azurerm_resource_group.vm_rg.name}"
  virtual_machine_name = "${azurerm_virtual_machine.vm_example.name}"
  ...
  settings = <<SETTINGS
  {
  "fileUris": ["https://sag.blob.core.windows.net/sagcont/install_terratest.sh"],
    "commandToExecute": "sh install_terratest.sh"
  }
SETTINGS
